 ...
 * 接口方法上的参数及其返回值类型都是自定义的,参数数量也没有任何要求,框架不做要求,如果交互使用的数据格式是<br/>
 * 对象序列化(即application/x-java-serialized- object)格式<br/>
 * 则要求参数和返回值类型都必须实现序列化接口{@link Serializable}<br/>
 * 接口的定义可以完全参照java的接口定义即可,唯一一个要求就是接口必须声明{@link IOException}或者更宽松的异常<br/>
...
public interface SimpleAPI {
	 ...
	 * 而造成的异常,所以接口上必须定义抛出异常,让客户端必须处理是比较安全的方案. 方法上必须定义 {@link IOException}
	 * 或者范围更宽的异常,如{@link Exception}和{@link Throwable}
	 * 
	 * @param hi
	 * @return
	 * @throws Exception
	...
	/**
	 * 因为默认情况下,注解{@link Open#all()}
	 * 属性是true,意味着接口上得所有方法都是开放且可远程调用的,如果想其中一些方法只允许在内部使用, 这时候注解{@link Close}<br/>
